...
Importer.prototype.getStatsString = function() {
  ...
  +"] \nAverage insert speed " +
    this.stats.speedAvg.toString().substr(0, 5)
  +"ms\nMemory usage " +
...
Importer.prototype.printStats = function() {
  this.options.stats && this.logger.info(this.getStatsString())
}
...
Importer.prototype.isClosed = function() {
  return this.parser.parserClosed
}
        ...
        return driver.save(queue.get(len || null))
          .then(function() {
            ...
            if(me.options.stats) {
              ...
              var insertTime = (runlen/(len || me.queue.batchSize))

              me.logger.debug("Insert time %sms", insertTime )
              me.logger.debug("Queue len %s", queue.length)

            }

            me.stats.insertLen += insertTime
            ops--

            if(ops === 0 && (!me.isEnded() && !me.isStopped())) {
              me.printStats()
